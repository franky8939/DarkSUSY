...
out_CMS = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
out_HL = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
out_relative = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
out_diff = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)

Matrix_MNeul = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
Matrix_MNeuD = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
Matrix_MPhoD = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
Matrix_TcPhoD = np.zeros([len(Vect_MNeuL), len(Vect_MNeuD), len(Vect_MPhoD), len(Vect_TcPhoD)], dtype=int)
# print out_CMS
                    ...
                    if out_HL[i_MNeuL, i_MNeuD, i_MPhoD, i_TcPhoD] > 0:
                        ...
                        out_diff[i_MNeuL, i_MNeuD,
                                 i_MPhoD, i_TcPhoD] = float(out_CMS[i_MNeuL, i_MNeuD, i_MPhoD, i_TcPhoD]) / \
                                                      float(out_HL[i_MNeuL, i_MNeuD, i_MPhoD, i_TcPhoD]) * 100
                    # Otras variables
                    Matrix_MNeul[:, i_MNeuD, i_MPhoD, i_TcPhoD] = MNeuL
                    Matrix_MNeuD[i_MNeuL, :, i_MPhoD, i_TcPhoD] = MNeuD
                    Matrix_MPhoD[i_MNeuL, i_MNeuD, :, i_TcPhoD] = MPhoD
                    Matrix_TcPhoD[i_MNeuL, i_MNeuD, i_MPhoD, :] = TcPhoD

hf.close()

# Graphics of all data
def graphics3DHist(x, y, z,
                   position=None,
                   title=None,
                   x_label=None,
                   y_label=None,
    ...
    ax.bar3d(x.ravel(), y.ravel(), 0,
             ...
             z.ravel(), zsort='average', alpha=alpha)
    if zlim is None:
...
fig = plt.figure()
print np.max(out_HL)
for i in range(len(Vect_TcPhoD)):
    position = [len(Vect_TcPhoD), 4, 4 * i + 1]
    print str(position)
    fig = graphics3DHist(Matrix_MPhoD[0, :, :, i], Matrix_MNeuD[0, :, :, i], out_HL[0, :, :, i] / 100, position=position,
                         title="Usando Card CMS, para TcPhoD (mm) = " + str(Vect_TcPhoD[i]),
                         x_label="MPhoD (GeV)",
                         y_label="MNeuD (GeV)",
                         z_label="EvMuMu (%)",
                         zlim=np.max(out_HL) / 100, figure=fig)

    position = [len(Vect_TcPhoD), 4, 4 * i + 2]
    fig = graphics3DHist(Matrix_MPhoD[0, :, :, i], Matrix_MNeuD[0, :, :, i], out_CMS[0, :, :, i] / 100, position=position,
                         title="Usando Card HL, para TcPhoD (mm) = " + str(Vect_TcPhoD[i]),
                         x_label="MPhoD (GeV)",
                         y_label="MNeuD (GeV)",
                         z_label="EvMuMu (%)",
                         zlim=np.max(out_HL) / 100, figure=fig)

    position = [len(Vect_TcPhoD), 4, 4 * i + 3]
    fig = graphics3DHist(Matrix_MPhoD[0, :, :, i], Matrix_MNeuD[0, :, :, i], out_relative[0, :, :, i], position=position,
                         title="Comparando Card, para TcPhoD (mm) = " + str(Vect_TcPhoD[i]),
                         x_label="MPhoD (GeV)",
                         y_label="MNeuD (GeV)",
                         z_label="Error Relativo Porcentual (%)",
                         zlim=np.max(out_relative), figure=fig)

    position = [len(Vect_TcPhoD), 4, 4 * i + 4]
    fig = graphics3DHist(Matrix_MPhoD[0, :, :, i], Matrix_MNeuD[0, :, :, i], out_diff[0, :, :, i], position=position,
                         title="Comparando Card, para TcPhoD (mm) = " + str(Vect_MNeuD[i]),
                         x_label="MPhoD (GeV)",
                         y_label="TcPhoD (mm)",
...
fig.savefig("files_PDF/Event_all_histConst_TcPhoD.pdf")
